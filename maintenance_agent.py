...
def run():
    ...
    # Export tickets to data.json for dashboard
    try:
        import sqlite3, json
        conn = sqlite3.connect(CONFIG["DB_PATH"])
        conn.row_factory = sqlite3.Row
        rows = conn.execute("SELECT * FROM maintenance_tickets ORDER BY date_submitted DESC").fetchall()
        tickets = [dict(r) for r in rows]
        conn.close()
        with open("data.json", "w") as f:
            json.dump({"tickets": tickets, "updated": datetime.now().isoformat()}, f)
        log.info(f"  Exported {len(tickets)} tickets to data.json")
    except Exception as e:
        log.warning(f"  Could not export data.json: {e}")
